...
class Ball(pygame.sprite.Sprite):
    ...
    def collide(self, *args: Rect, player1, player2):
        ...
        if self.rect.left < 0:
            ...
            player1.raquette.rect.centery = SCREEN_HEIGHT /2
            player2.raquette.rect.centery = SCREEN_HEIGHT /2
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_F11:
                if FULLSCREEN:
                    ...
                    ball_pos_ratio_y = height() / ball1.rect.centery
                    pygame.display.set_mode((DEFAULT_SCREEN_WIDTH, DEFAULT_SCREEN_HEIGHT), pygame.RESIZABLE)
                    FULLSCREEN = False
                    ball1.rect.centerx = width() / ball_pos_ratio_x
                    ball1.rect.centery = height / ball_pos_ratio_y
                    text_rect = text.get_rect(center=win.get_rect().center)
                    win.blit(text, text_rect)
